...
1. 맵을 순회하며 빈자리를 찾는다.
<br/>
2. 해당자리 인근(상하좌우)에 빈칸과 좋아하는 친구의 수를 계산한다. 
<br/>
3. 반복문을 돌면서 우선순위가 더 높은 빈자리를 찾는다. 
<br/>
(좋아하는 학생 인접수 -> 인접한 빈칸수 -> 행과 열 기준)
<br/>
> 구현 문제에서 생각보다 for문을 많이 써야하는 경우가 존재하는데, 종종 작성을 망설이는 경우가 있다. 해당문제는 시간복잡도를 계산하기 어렵지 않았지만 시간초과에 대한 우려나 지저분한 코드의 느낌을 지울 수 없어서 그런것같다. 구현 문제에선 코드의 완성을 우선순위로 두면 좋을 것 같다. 