...
1. 그룹 블록을 선택하는 우선순위를 지정하는 로직을 구현하는데 실수 할만한 요소가 너무많다.
	- 블록 그룹에서 행과 열이 가장 작은 블록이 기준블록이 된다.
	- 그룹 블록의 사이즈가 가장 큰 블록을 선택한다. (일반적인 BFS)
	- 2번이 동일할 경우 무지개색 블록이 가장 많은 그룹을 선택한다. (BFS가 끝나면 visit = true가 되기 때문에 다른 블록에서도 쓸 수 있도록 다시 false로 바꿔주는 로직이 필요하다.)
	- 3번이 동일할 경우 기준블록의  행이 가장 큰 그룹을 선택한다. 행도 같다면 열이 가장 큰 그룹을 선택한다. (기준 블록 듣자마자 행과 열이 가장 작은것으로 착각 할 수 있다.)
<br/>
2. 위의 요소들을 신경쓰면서 다음과정을 반복한다.
	- BFS를 반복적으로 진행하면서 우선순위에 맞게 블록 그룹을 선택한다.
		> 블록수, 무지개블록수, 행, 열을 계속해서 기록하며 우선순위에 맞게 업데이트 해준다.
	- destroyBfs() 함수를 가지고, 해당 그룹블록을 모두 파괴한다.
	- gravity() → turn() → gravity() 를 반복
		> gravity()가 N^3의 시간복잡도를 가지는데, 한변이 최대 20인 점을 고려하면 비용이 크지 않아 쫄지말고 작성하는게 중요하다. 특히 구현에서는..
	- 2개 이상의 블록으로 구성된 그룹이 없다면 while문을 종료한다.
